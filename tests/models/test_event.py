...
def _sample_calendar() -> Calendar:
    return Calendar(name="work", carrie="cal_carrie", larry="cal_larry", shared="")
...
def test_event_from_dict() -> None:
    ...
    event = Event.from_dict(raw, calendar=cal, owner=Owner.larry)
    assert event.summary == "Dentist"
    assert event.location is None
    assert event.event_id == "evt_123"
    assert event.calendar is cal
    assert event.owner is Owner.larry
