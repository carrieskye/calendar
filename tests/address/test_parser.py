...
def test_uk_cardiff_full_string() -> None:
    raw = "25a Bromsgrove St, Cardiff CF11 7EZ, United Kingdom"
    addr = AddressParser.run(raw)
    assert isinstance(addr, UKAddress)
    assert addr.city == "Cardiff"
    assert addr.postal_code == "CF11 7EZ"
    assert addr.street == "Bromsgrove St"
    assert addr.house_no == "25a"
...
@pytest.mark.parametrize(
    "raw",
    [
        "25a Bromsgrove St, Cardiff CF11 7EZ, United Kingdom",
        "25a Bromsgrove St, Cardiff CF11 7EZ, UK",
    ],
)
