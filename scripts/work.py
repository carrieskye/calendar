...
class AddDays(Work):
    ...
    def __init__(self):
        ...
        self.addresses = {
            'Bromsgrove': '25 Bromsgrove St, Cardiff CF10 1AB, UK',
    ...
    def run(self):
        ...
        while self.day <= self.end:
            if self.day.weekday() == 7 or self.day.weekday() < 5:
                ...
                self.create_event(self.cal_id_work_partner, 'Amplyfi', 'Bromsgrove', '', time(10), time(12, 30))

                self.create_event(self.cal_id_food_carrie, 'Lunch', 'Bromsgrove', '', time(12, 30), time(13, 30))
                self.create_event(self.cal_id_food_partner, 'Lunch', 'Bromsgrove', '', time(12, 30), time(13, 30))

                self.create_event(self.cal_id_work, 'Amplyfi', 'Bromsgrove', self.project, time(13, 30), time(17))
                self.create_event(self.cal_id_work_partner, 'Amplyfi', 'Bromsgrove', '', time(13, 30), time(17))
...
class CopyToLarry(Work):
    ...
    def run(self):
        ...
        print(self.cal_id_work_partner)
        print(self.start)
        print(self.end)

        results = self.google_cal.get_events(self.cal_id_work_partner, 1000, self.start, self.end)
        for result in results:
            event = Event.get_event(result, 'work_partner')
            self.google_cal.delete_event(self.cal_id_work_partner, event.event_id)
        ...
        for result in results:
            ...
            event.description = ''
            self.google_cal.create_event(self.cal_id_work_partner, event)
