...
class LocationUtils:
    ...
    def get_records(start: datetime, end: datetime, owner: Owner, accuracy: int = 20):
        user_id = 3 if owner == Owner.user else 2
    ...
    @classmethod
    def process_events(cls, start: date, history: List[LocationEventTemp], partner: bool):
        # TODO fix
        ...
        for event in events:
            ...
            if popped_event:
                ...
                if event.calendar != 'work' or not partner:
        ...
        for remaining_entry in history:
            ...
            if not (too_short or home):
                ...
                if not unknown:
                    ...
                    GoogleCalAPI.create_event(Calendars.various.user, event)
