...
class LocationScript(Script, ABC):
    @classmethod
    def get_location_timestamps(
        cls, start: datetime, end: datetime, owner: Owner
    ) -> LocationTimestamps:
    ...
    def get_closest_location(cls, location_timestamp: LocationTimestamp) -> str:
        ...
        if len(matches) > 1:
            ...
            for match in matches:
                ...
                distances[match] = cls.get_distance(
                    location_timestamp.get_point(), intersection
                )
    ...
    def filter_geo_locations(cls, location_timestamp: LocationTimestamp):
        ...
        for label, geo_location in Data.geo_location_dict.items():
            if geo_location.bounding_box:
                point_a = Point(
                    location_timestamp.latitude, location_timestamp.longitude
                )
                if (
                    cls.get_distance(geo_location.bounding_box.intersection, point_a)
                    < 2 * location_timestamp.latitude
                ):
