    ...
    def run(self):
        ...
        while self.day <= self.end:
            if self.day.weekday() == 7 or self.day.weekday() < 5:
                ...
                # self.create_event(Calendars.work.carrie, 'Amplyfi', self.location, start=time(10),
                # end=time(12))
                self.create_event(
                    Calendars.work.larry,
                    "Delio",
                    self.location,
                    start=time(9),
                    end=time(12),
                )
                # self.create_event(Calendars.food.carrie, 'Lunch', self.location, time(12),
                # time(13))
                self.create_event(
                    Calendars.food.larry, "Lunch", self.location, time(12), time(12, 30)
                )
                # self.create_event(Calendars.work.carrie, 'Amplyfi', self.location, start=time(13),
                # end=time(17))
                self.create_event(
                    Calendars.work.larry,
                    "Delio",
