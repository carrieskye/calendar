...
class AddDays(WorkScript):
    def __init__(self):
        ...
        self.end = self.day + relativedelta(
            days=Input.get_int_input("Days", "#days") - 1
        )
        ...
        while self.day <= self.end:
            if self.day.weekday() == 7 or self.day.weekday() < 5:
                if self.skating:
                    ...
                    self.create_event(
                        Calendars.sports.shared, "Ice skating", viola, time(8), time(9)
                    )

                # self.create_event(Calendars.work.user, 'Amplyfi', self.location, start=time(10),
                # end=time(12))
                self.create_event(
                    Calendars.work.partner,
                    "Delio",
                    self.location,
                    start=time(9),
                    end=time(12),
                )
                # self.create_event(Calendars.food.user, 'Lunch', self.location, time(12),
                # time(13))
                self.create_event(
                    Calendars.food.partner, "Lunch", self.location, time(12), time(12, 30)
                )
                # self.create_event(Calendars.work.user, 'Amplyfi', self.location, start=time(13),
                # end=time(17))
                self.create_event(
                    Calendars.work.partner,
                    "Delio",
