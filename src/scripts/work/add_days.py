...
class AddDays(Work):
    ...
    def __init__(self):
        ...
        self.project = Input.get_string_input('Project', default='Analyse')
    ...
    def create_work_event(self, location: GeoLocation, start: time, end: time):
        for calendar in [Calendars.work.user, Calendars.work.partner]:
            desc = self.project if calendar == Calendars.work.user else ''
    ...
    def create_lunch_event(self, location: GeoLocation, start: time, end: time):
        for calendar in [Calendars.food.user, Calendars.food.partner]:
