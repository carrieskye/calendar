...
class ParseTimingExportScript(Work):
    ...
    def run(self):
        ...
        export = Utils.read_csv('data/activity/user.csv')
        ...
        for day, activities in activities_per_day.items():
            ...
            Utils.write_csv([x.flatten() for x in activities], f'data/activity/user/csv/{day}.csv')
            Utils.write_json(json.loads(jsonpickle.encode(activities)), f'data/activity/user/json/{day}.json')
