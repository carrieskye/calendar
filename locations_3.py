...
def get_records(date: str):
    query = f'SELECT * FROM public.positions where time > \'{date}\' AND user_id = 3'
...
def get_closest_location(lat, lon, accuracy, previous_name):
    ...
    for title, frequent_location in FREQUENT_LOCATIONS.items():
        ...
        distances[title] = get_distance(lat, lon, location_lat, location_lon)
...
def process_events(year: int, month: int, day: int, history: List[dict], partner: bool):
    events = get_day_events(year, month, day)
    for event in events:
        popped_event = get_closest_event(event, history)
        if popped_event and (event.calendar != 'work' or not partner):
...
def close_to_bad_locations(latitude, longitude):
    for bad_location in BAD_LOCATIONS:
        if get_distance(latitude, longitude, bad_location.get('latitude'), bad_location.get('longitude')) < 20:
            return True
    return False


def run(year: str, month: str, day: str, partner: str = 'false'):
    year, month, day = (int(year), int(month), int(day))
    results = get_records(parse(datetime(year, month, day).isoformat()))
    ...
    for entry in history:
        ...
        print(entry.get('start').astimezone(time_zone), entry.get('end').astimezone(time_zone),
              entry.get('location'))
    print()
    process_events(year, month, day, history, strtobool(partner))


if __name__ == '__main__':
    day = input(f'Date ({TODAY}): ')
    day = parse(day) if day else parse(TODAY)
    run()
    if len(sys.argv) > 2:
