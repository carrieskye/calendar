...
class Clean:
    ...
    def clean_old_events(items: int, start: datetime, end: datetime, calendars: str):
        ...
        for calendar_name, calendar_id in calendars.items():
            if start >= datetime(2018, 1, 1) or 'partner' not in calendar_name:
...
class Print:
    ...
    def print_history(items: int, start: datetime, end: datetime, calendars: str, sort_by: List[str], split_cal: bool,
        ...
        if split_cal:
            Print.print_history_per_calendar(items, start, end, calendars, sort_by, partner)
        else:
            Print.print_history_together(items, start, end, calendars, sort_by, partner)
    ...
    def print_history_per_calendar(items: int, start: datetime, end: datetime, calendars: dict, sort_by: List[str],
        ...
        for calendar_name, calendar_id in calendars.items():
            if calendar_name in calendars:
                if partner or 'partner' not in calendar_name:
                    results = GOOGLE_CALENDAR_API.get_events(calendar_id, items, start, end)
...
def print_history_year(year=CURRENT_YEAR, calendars='all', sort_by='start.date_time', split_cal='true', partner='true'):
    GOOGLE_CALENDAR_API.update_calendar_dict()
    start = datetime(int(year), 1, 1)
    end = start + relativedelta(years=1)
    sort_by = sort_by.split(',') if sort_by else []
    Print.print_history(10000, start, end, calendars, sort_by, strtobool(split_cal), strtobool(partner))


def print_full_history(calendars='all', sort_by='start.date_time', split_cal='true', partner='true'):
...
def print_history_as_of(start: str, calendars='all', sort_by='start.date_time', split_cal='true', partner='true'):
    start = parse(start)
